...
const EbookManagement = () => {
  ...
  useEffect(() => {
    const fetchEbooks = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/book/ebooks", {
  ...
  const handleUpdatePdfFileChange = (e) => {
  ...
  const addEbook = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/api/book/ebooks",
        formData,
        {
          headers: {
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(
        `http://localhost:5000/api/book/ebooks/${selectedEbook._id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const updateEbook = async () => {
    try {
      ...
      const res = await axios.put(
        `http://localhost:5000/api/book/ebooks/${editEbook._id}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={{ fontWeight: "bold" }}>E-Book Cover</TableCell>
              ...
              {filteredBooks.map((book, index) => (
                <TableRow key={book._id}>
                  ...
                  <TableCell>
                    <Avatar
                      alt="Book Cover"
                      src={`http://localhost:5000/public/${book.cover_path}`}
                      variant="square"
                      sx={{
                        width: 80,
                        height: 95,
