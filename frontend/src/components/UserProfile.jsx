...
const UserProfile = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/librarian/me", {
  ...
  const handleUpdate = async () => {
    try {
      await axios.put("http://localhost:5000/api/librarian/me", userData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setEditMode(false);
  ...
  const handleDelete = async () => {
    try {
      await axios.delete("http://localhost:5000/api/librarian/me", {
        headers: { Authorization: `Bearer ${token}` },
      });
      logout();
      navigate("/login");
